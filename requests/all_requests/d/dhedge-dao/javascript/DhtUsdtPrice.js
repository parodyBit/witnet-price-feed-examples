...
// Retrieves DHT/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves USDT price of DHT from the OkEx API (derived from USDT/USD exchange rate)
const okex = new Witnet.Source("https://www.okex.com/api/index/v3/DHT-USDT/constituents")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
// Retrieves USDT price of DHT from the HOTBIT API
const hotbit = new Witnet.Source("https://api.hotbit.io/api/v1/market.last?market=DHTUSDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("result") // Get the `Float` value associated to the `result` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer

// Retrieves DHT/USDT price from the HUOBI API
const huobi = new Witnet.Source("https://api.huobi.pro/market/detail/merged?symbol=dhtusdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
