...
// Retrieves DOSE/USDT price from the HUOBI API
const huobi = new Witnet.Source("https://api.huobi.pro/market/detail/merged?symbol=doseusdt")
...
// Retrieves USDT price of DOSE from the OkEx API (derived from USDT/USD exchange rate)
const okex = new Witnet.Source("https://www.okex.com/api/index/v3/DOSE-USDT/constituents")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
// Retrieves DOSE/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves USDT price of DOSE from the Gate.io API
const gateio = new Witnet.Source("https://data.gateapi.io/api2/1/ticker/dose_usdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("last") // Get the `Float` value associated to the `last` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer
