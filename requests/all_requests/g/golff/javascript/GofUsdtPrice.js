...
// Retrieves GOF/USDT price from the HUOBI API
const huobi = new Witnet.Source("https://api.huobi.pro/market/detail/merged?symbol=gofusdt")
...
// Retrieves USDT price of GOF from the HOTBIT API
const hotbit = new Witnet.Source("https://api.hotbit.io/api/v1/market.last?market=GOFUSDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("result") // Get the `Float` value associated to the `result` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer

// Retrieves GOF/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves USDT price of GOF from the Gate.io API
const gateio = new Witnet.Source("https://data.gateapi.io/api2/1/ticker/gof_usdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
