...
// Retrieves RON/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves USDT price of RON from the OkEx API (derived from USDT/USD exchange rate)
const okex = new Witnet.Source("https://www.okex.com/api/index/v3/RON-USDT/constituents")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
// Retrieves USDT price of RON from the Gate.io API
const gateio = new Witnet.Source("https://data.gateapi.io/api2/1/ticker/ron_usdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("last") // Get the `Float` value associated to the `last` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer
