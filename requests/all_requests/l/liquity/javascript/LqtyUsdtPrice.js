...
// Retrieves LQTY/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves LQTY/USDT price from the HUOBI API
const huobi = new Witnet.Source("https://api.huobi.pro/market/detail/merged?symbol=lqtyusdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
// Retrieves USDT price of LQTY from the HOTBIT API
const hotbit = new Witnet.Source("https://api.hotbit.io/api/v1/market.last?market=LQTYUSDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("result") // Get the `Float` value associated to the `result` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer

// Retrieve LQTY/USDT-6 price from Coinbase
