...
// Retrieves NEST/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves USDT price of NEST from the HOTBIT API
const hotbit = new Witnet.Source("https://api.hotbit.io/api/v1/market.last?market=NESTUSDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("result") // Get the `Float` value associated to the `result` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer

// Retrieves NEST/USDT price from the HUOBI API
const huobi = new Witnet.Source("https://api.huobi.pro/market/detail/merged?symbol=nestusdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
// Retrieves USDT price of NEST from the Gate.io API
const gateio = new Witnet.Source("https://data.gateapi.io/api2/1/ticker/nest_usdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
