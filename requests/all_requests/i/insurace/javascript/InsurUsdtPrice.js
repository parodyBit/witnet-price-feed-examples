...
// Retrieves INSUR/USDT price from the HUOBI API
const huobi = new Witnet.Source("https://api.huobi.pro/market/detail/merged?symbol=insurusdt")
...
// Retrieves INSUR/USDT-6 from the Hoo HTTP-GET API
...
// Retrieves USDT price of INSUR from the HOTBIT API
const hotbit = new Witnet.Source("https://api.hotbit.io/api/v1/market.last?market=INSURUSDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getFloat("result") // Get the `Float` value associated to the `result` key
  .multiply(10 ** 6) // Use 6 digit precision
  .round() // Cast to integer

// Retrieves USDT price of INSUR from the Gate.io API
const gateio = new Witnet.Source("https://data.gateapi.io/api2/1/ticker/insur_usdt")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
